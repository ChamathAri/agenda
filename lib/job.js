...
Job.prototype.computeNextRunAt = function () {
    ...
    function computeFromInterval() {
        ...
        try {
            ...
            //var nextDates = cronTime._getNextDateFrom(lastRun);
            //console.log("cron next date -->" + nextDates.valueOf());

            var cronSched = later.parse.cron(interval);
            var nextDate = later.schedule(cronSched).next(1,lastRun.format());
            nextDate = dateForTimezone(nextDate[0]);
            //console.log("next date -->" + nextDate.valueOf());
            //console.log("last run date -->" + lastRun.valueOf());
            ...
            if (nextDate.valueOf() == lastRun.valueOf()) {
                ...
                nextDate = later.schedule(cronSched).next(1,k.format());
                nextDate = dateForTimezone(nextDate[0]);
            }
