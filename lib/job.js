...
Job.prototype.computeNextRunAt = function () {
    ...
    function computeFromInterval() {
        ...
        try {
            ...
            var substring = "$";
            var nextDate = "";
            if (interval.includes(substring)) {
                var intervalArr = interval.split(" ");
                var minutes = intervalArr[0];
                var hour = intervalArr[1];
                var weekStr = intervalArr[4];
                var dayOfWeekStr = weekStr.split("$")[0];
                var recur_weeks = weekStr.split("$")[1];
                var daysOfWeekToNumbers = getDaysToNumberString(dayOfWeekStr.split(","));

                var s = getLaterExpForWeeks(minutes, hour, recur_weeks, daysOfWeekToNumbers);
                nextDate = later.schedule(s).next(2, lastRun);
                nextDate = dateForTimezone(nextDate[1]);

            } else {
                var cronSched = later.parse.cron(interval, false);
                nextDate = later.schedule(cronSched).next(2, lastRun);
                nextDate = dateForTimezone(nextDate[1]);
            }
            ...
            if (nextDate.valueOf() == lastRun.valueOf()) {
                ...
                var k = dateForTimezone(new Date(lastRun.valueOf() + 1000));
                if (interval.includes(substring)) {
                    nextDate = later.schedule(s).next(2, k);
                    nextDate = dateForTimezone(nextDate[1]);
                } else {
                    nextDate = later.schedule(cronSched).next(2, k);
                    nextDate = dateForTimezone(nextDate[1]);
                    console.log(nextDate.format());
                }
            }
    ...
    function getLaterExpForWeeks(min, hour, recur_weeks, daysOfWeekToNumbers) {
        var days = daysOfWeekToNumbers.split(",");
        recur_weeks = parseInt(recur_weeks);
        hour = parseInt(hour);
        min = parseInt(min);
        switch (days.length) {
            case 1:
                return later.parse.recur().every(recur_weeks).weekOfYear().on(parseInt(days[0])).dayOfWeek().on(hour).hour().on(min).minute();
            case 2:
                return later.parse.recur().every(recur_weeks).weekOfYear().on(parseInt(days[0]), parseInt(days[1])).dayOfWeek().on(hour).hour().on(min).minute();
            case 3:
                return later.parse.recur().every(recur_weeks).weekOfYear().on(parseInt(days[0]), parseInt(days[1]), parseInt(days[2])).dayOfWeek().on(hour).hour().on(min).minute();
            case 4:
                return later.parse.recur().every(recur_weeks).weekOfYear().on(parseInt(days[0]), parseInt(days[1]), parseInt(days[2]), parseInt(days[3])).dayOfWeek().on(hour).hour().on(min).minute();
            case 5:
                return later.parse.recur().every(recur_weeks).weekOfYear().on(parseInt(days[0]), parseInt(days[1]), parseInt(days[2]), parseInt(days[3]), parseInt(days[4])).dayOfWeek().on(hour).hour().on(min).minute();
            case 6:
                return later.parse.recur().every(recur_weeks).weekOfYear().on(parseInt(days[0]), parseInt(days[1]), parseInt(days[2]), parseInt(days[3]), parseInt(days[4]), parseInt(days[5])).dayOfWeek().on(hour).hour().on(min).minute();
            case 7:
                return later.parse.recur().every(recur_weeks).weekOfYear().on(parseInt(days[0]), parseInt(days[1]), parseInt(days[2]), parseInt(days[3]), parseInt(days[4]), parseInt(days[5]), parseInt(days[6])).dayOfWeek().on(hour).hour().on(min).minute();
        }
    }

    function getDaysToNumberString(days_of_week) {
        var daysToNumberString = "";
        for (var i = 0; i < days_of_week.length; i++) {
            daysToNumberString += mapDaysToNumbers(days_of_week[i]) + ",";
        }
        daysToNumberString = daysToNumberString.slice(0, -1);
        daysToNumberString = daysToNumberString.split(",");

        var sortedDayToNumbersString = daysToNumberString.sort((a, b) => (a - b));
        sortedDayToNumbersString = sortedDayToNumbersString.join(",");
        return sortedDayToNumbersString;
    }
